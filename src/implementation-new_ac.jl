...
function set_model_objective_function!(power_flow_model::AbstractMPOPFModel, factory::NewACMPOPFModelFactory)
    ...
    #Adding contraints in LHS - RHS form to add to objective function
    for t in 1:T
        for (i, branch) in ref[:branch]
            ...
            p_fr_new += p_fr - ( (g+g_fr)/ttm*vm_fr^2 + (-g*tr+b*ti)/ttm*(vm_fr*vm_to*cos(va_fr-va_to)) + (-b*tr-g*ti)/ttm*(vm_fr*vm_to*sin(va_fr-va_to)) )
            q_fr_new += q_fr - (-(b+b_fr)/ttm*vm_fr^2 - (-b*tr-g*ti)/ttm*(vm_fr*vm_to*cos(va_fr-va_to)) + (-g*tr+b*ti)/ttm*(vm_fr*vm_to*sin(va_fr-va_to)) )
            p_to_new += p_to - ( (g+g_to)*vm_to^2 + (-g*tr-b*ti)/ttm*(vm_to*vm_fr*cos(va_to-va_fr)) + (-b*tr+g*ti)/ttm*(vm_to*vm_fr*sin(va_to-va_fr)) )
            q_to_new += q_to - (-(b+b_to)*vm_to^2 - (-b*tr+g*ti)/ttm*(vm_to*vm_fr*cos(va_to-va_fr)) + (-g*tr-b*ti)/ttm*(vm_to*vm_fr*sin(va_to-va_fr)) )
        end
    end
    
    # Added contraints above to objective function
    @objective(model, Min,
        ...
        (p_fr_new + q_fr_new + p_to_new +q_to_new)
    )
end
