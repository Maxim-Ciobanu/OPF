module MPOPF
    ...
    export create_model_ac, optimize_model
    ...
    function PowerFlowModel(file_path, optimizer, time_periods=1, ramping_cost=0)::PowerFlowModel
        ...
        return PowerFlowModel(model, data, time_periods, ramping_cost)
    end

    function set_model_variables_ac!(power_flow_model::PowerFlowModel)
    ...
    function set_model_objective_function_ac!(power_flow_model::PowerFlowModel)
        model = power_flow_model.model
    ...
    function set_model_constraints_ac!(power_flow_model::PowerFlowModel)
        model = power_flow_model.model
        data = power_flow_model.data
        T = power_flow_model.time_periods
        ref = PowerModels.build_ref(data)[:it][:pm][:nw][0]
    ...
    function create_model_ac(file_path::String, optimizer, time_periods=1, ramping_cost=0)::PowerFlowModel
        
        my_model = MPOPF.PowerFlowModel(file_path, optimizer, time_periods, ramping_cost)

        set_model_variables_ac!(my_model)

        set_model_objective_function_ac!(my_model)

        set_model_constraints_ac!(my_model)
    ...
    function optimize_model(model::PowerFlowModel)
        println("Processing AC Model with case data: ", model.data)
